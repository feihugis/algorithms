...
public class Word_Ladder_II_126 {
  ...
  public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
    ...
    depth = wordList.size() + 1;
    HashSet<String> set = new HashSet<>();
    set.addAll(wordList);
    List<String> cur = new ArrayList<>();
    cur.add(beginWord);

    findLadders(beginWord, endWord, set, cur, result);

    List<List<String>> shortestResult = new ArrayList<>();
    for (List<String> r : result) {
      if (r.size() == depth) {
        shortestResult.add(r);
      }
    }
    return shortestResult;
  }

  public void findLadders(String beginWord, String endWord, HashSet<String> wordList,
                          List<String> list, List<List<String>> result) {
    if (depth < list.size()) return;
    System.out.println(Arrays.toString(list.toArray()));
    if (list.contains(endWord)) {
      depth = list.size() < depth? list.size() : depth;
    ...
    for (String candidate : candidates) {
      list.add(candidate);
      findLadders(candidate, endWord, wordList, list, result);
      list.remove(list.size() - 1);
    }
    wordList.addAll(removed);
  }

  public static void main(String[] args) {
    Word_Ladder_II_126 solution = new Word_Ladder_II_126();
